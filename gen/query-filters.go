...
func (qf *ExportQueryFilter) Apply(ctx context.Context, dialect gorm.Dialect, selectionSet *ast.SelectionSet, wheres *[]string, values *[]interface{}, joins *[]string) error {
	...
	for _, part := range queryParts {
		ors := []string{}
		if err := qf.applyQueryWithFields(dialect, fields, part, TableName("exports"), &ors, values, joins); err != nil {
			return err
		}
...
func (qf *ExportQueryFilter) applyQueryWithFields(dialect gorm.Dialect, fields []*ast.Field, query, alias string, ors *[]string, values *[]interface{}, joins *[]string) error {
	if len(fields) == 0 {
		return nil
	}
	...
	if _, ok := fieldsMap["metadata"]; ok {

		column := dialect.Quote(alias) + "." + dialect.Quote("metadata")

		*ors = append(*ors, fmt.Sprintf("%[1]s LIKE ? OR %[1]s LIKE ?", column))
		*values = append(*values, query+"%", "% "+query+"%")
	}

	if _, ok := fieldsMap["progress"]; ok {

		cast := "TEXT"
		if dialect.GetName() == "mysql" {
			cast = "CHAR"
		}
		column := fmt.Sprintf("CAST(%s"+dialect.Quote("progress")+" AS %s)", dialect.Quote(alias)+".", cast)
